...
def process_cah_data_for_finetuning(df):
    ...
    for _, black_row in black_cards.iterrows():
        ...
        # I randomly sampled 5 white cards for each black card, but that's not the right approach, use actual played combinations.
        ...
        for _, white_row in sample_white_cards.iterrows():
            ...
            # Create input-output pair - T5 expects a "task:" prefix
            input_text = f"complete card: {black_text}"
...
def finetune_model(args):
    """Fine-tune a T5 model on CAH data."""
    # Load data
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune a T5 model on CAH data")
    ...
    parser.add_argument("--model_name", type=str, default="t5-base",
                       help="Model to fine-tune (e.g., t5-small, t5-base)")
