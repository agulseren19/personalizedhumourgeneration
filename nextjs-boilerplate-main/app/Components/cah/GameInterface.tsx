...
import { Play, Sparkles, ThumbsUp, ThumbsDown, RotateCcw, Users, Gamepad2, Circle, PartyPopper, Target, Gamepad, AlertTriangle, Scale, Rocket, Briefcase, Globe, Laptop, Heart, Pizza, Home, CheckCircle, Link } from 'lucide-react';
...
export default function GameInterface({ userId }: GameInterfaceProps) {
  ...
  useEffect(() => {
    ...
    setBlackCardTopic('');
    setGameCardRatings({});
    // Note: Game mode removed, always clear game-related state
  ...
  const audienceOptions = [
    { value: 'friends', label: <><Users size={16} className="inline mr-1" /> Friends</>, description: 'Casual and fun' },
    { value: 'colleagues', label: <><Briefcase size={16} className="inline mr-1" /> Colleagues</>, description: 'Professional humor' },
    { value: 'family', label: <><Home size={16} className="inline mr-1" /> Family</>, description: 'Family-friendly' },
    { value: 'general', label: <><Globe size={16} className="inline mr-1" /> General</>, description: 'Broad audience' }
  ];

  const topicOptions = [
    { value: 'general', label: <><Target size={16} className="inline mr-1" /> General</> },
    { value: 'work', label: <><Briefcase size={16} className="inline mr-1" /> Work</> },
    { value: 'technology', label: <><Laptop size={16} className="inline mr-1" /> Technology</> },
    { value: 'relationships', label: <><Heart size={16} className="inline mr-1" /> Relationships</> },
    { value: 'food', label: <><Pizza size={16} className="inline mr-1" /> Food</> },
    { value: 'gaming', label: <><Gamepad2 size={16} className="inline mr-1" /> Gaming</> },
    { value: 'lifestyle', label: <><Home size={16} className="inline mr-1" /> Lifestyle</> }
  ];
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      } else if (isAlreadyBlackCard && (cardType === 'white' || cardType === 'multiplayer')) {
  ...
  return (
    <div className="space-y-8">
      {/* Game Setup */}
      <div className="bg-text-primary/10 backdrop-blur-sm rounded-xl p-6 border border-text-secondary/20">
        {user && (
          <div className="mb-6 p-4 bg-gradient-to-r from-accent-turquoise/20 to-accent-pink/20 rounded-lg border border-accent-turquoise/30">
            <h3 className="text-lg font-semibold text-accent-turquoise mb-2">
        ...
        {/* Card Type Selector */}
        <div className="mb-6">
          <label className="block text-text-primary font-medium mb-2">Card Type</label>
          <div className="flex space-x-4">
            <button
              onClick={() => setCardType('multiplayer')}
              className={`px-4 py-2 rounded-lg font-medium transition-all ${
                cardType === 'multiplayer'
                  ? 'bg-accent-turquoise text-text-primary shadow-lg'
                  : 'bg-text-primary/20 text-background-cream hover:bg-text-primary/30'
              }`}
            >
        ...
        {cardType === 'multiplayer' && (
          <div className="mb-6">
            ...
            {gameId && (
              <div className="bg-gradient-to-r from-accent-pink/20 to-accent-turquoise/20 border border-accent-pink/30 rounded-lg p-4 mb-4">
                <div className="text-center">
                  <div className="text-accent-pink font-medium mb-2">
                    <Gamepad className="inline mr-2" size={16} />
                    Active Game
                  </div>
                  ...
                  <div className="bg-white/20 rounded-lg p-4 border border-blue-500/50">
                    <div className="text-3xl font-bold text-accent-pink font-mono tracking-widest mb-3">
                      {gameId}
                    </div>
                    <div className="flex gap-2 justify-center">
                      <button
                        ...
                        className="px-4 py-2 bg-blue-600 text-text-primary text-sm rounded hover:bg-blue-700 transition-all"
                      >
                        📋 Copy Game ID
                      </button>
                      <button
                        ...
                        className="px-4 py-2 bg-accent-green text-white text-sm rounded hover:bg-accent-darkGreen transition-all"
                      >
                        📤 Share Game
                      </button>
                    </div>
            ...
            {!isInGame ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white/10 rounded-lg p-4">
                    <h3 className="text-lg font-bold text-text-primary mb-3">
                      <Gamepad className="inline mr-2" size={18} />
                      Create New Game
                    </h3>
                  ...
                  <div className="bg-white/10 rounded-lg p-4">
                    ...
                    <div className="mb-3">
                      <input
                        ...
                        className="w-full p-3 rounded-lg bg-white/20 text-white placeholder-gray-300 border border-gray-400 focus:border-green-400 focus:outline-none"
                      />
                    </div>
                    <button
                      onClick={joinGame}
                      disabled={loading || !user || !gameId.trim()}
                      className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-text-primary font-bold py-3 px-6 rounded-lg hover:from-blue-500 hover:to-blue-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                    >
              ...
              <div className="space-y-4">
                ...
                <div className="bg-green-500/20 border border-green-500/30 rounded-lg p-4">
                  <div className="text-accent-turquoise font-medium mb-2">
                    <Gamepad className="inline mr-2" size={16} />
                    Game in Progress
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="bg-white/20 rounded-lg p-2 border border-green-500/50">
                        <div className="text-lg font-bold text-accent-turquoise font-mono tracking-wider text-center">
                          {gameId}
                        </div>
                      </div>
                      <button
                        ...
                        className="mt-2 w-full px-3 py-1 bg-accent-turquoise text-white text-xs rounded hover:bg-green-700 transition-all"
                      >
                        📋 Copy
                      </button>
                    </div>
                ...
                {/* Game Status */}
                <div className="bg-white/10 rounded-lg p-4">
                  <h3 className="text-lg font-bold text-text-primary mb-2">Game Status: {gameState.status.toUpperCase()}</h3>
                  ...
                  <div className="mt-3 pt-3 border-t border-white/20">
                    <div className="text-sm text-accent-turquoise mb-2">
                ...
                {/* Players List */}
                <div className="bg-white/10 rounded-lg p-4">
                  <h3 className="text-lg font-bold text-text-primary mb-2">Players ({gameState.players?.length || 0})</h3>
                  <div className="space-y-2">
                    {gameState.players && Array.isArray(gameState.players) ? gameState.players.map((player) => (
                      <div key={player.user_id} className="flex justify-between items-center text-text-primary">
                        <span className="flex items-center">
                          {player.username}
                          {player.is_host && <span className="ml-2 text-yellow-400">👑</span>}
                          {player.is_judge && <span className="ml-2 text-accent-pink"><Scale size={16} /></span>}
                ...
                {gameState.status === 'waiting' && gameState.players && Array.isArray(gameState.players) && gameState.players.find(p => p.is_host)?.user_id === (gameUserId || userId) && (
                  <div className="space-y-2">
                    ...
                    >
                      {gameState.players.length >= 2 
                        ? <><Rocket className="inline mr-2" size={16} />Start Game!</> 
                ...
                {gameState.current_round && (
                  <div className="bg-white/10 rounded-lg p-4">
                    ...
                    <div className="bg-black text-text-primary p-4 rounded-lg mb-4 text-center font-bold">
                    ...
                    {gameState.current_round.phase === 'card_submission' && 
                     ...
                     gameState.current_round?.judge_id !== (gameUserId || userId) && (
                      <div>
                        <h4 className="text-text-primary font-medium mb-2">Your Cards:</h4>
                    ...
                    {gameState.current_round.phase === 'card_submission' && 
                     gameState.current_round?.judge_id === (gameUserId || userId) && (
                      <div className="text-center text-text-secondary py-4">
                        ...
                        {gameState.current_round.submissions && gameState.current_round.submissions.length > 0 && (
                          <div className="mt-4">
                            <h5 className="text-text-primary font-medium mb-2">Submitted Cards:</h5>
                            <div className="space-y-2">
                              {gameState.current_round.submissions.map((submission, index) => (
                                <div key={index} className="bg-white/20 text-text-primary p-3 rounded-lg border border-text-secondary/30">
                                  <span className="text-sm text-text-secondary">{submission.player_username || `Player ${index + 1}`}:</span>
                                  <p className="text-text-primary mt-1">{submission.card}</p>
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    {gameState.current_round.phase === 'judging' && 
                     gameState.current_round.judge_id === (gameUserId || userId) && (
                      <div>
                        <h4 className="text-text-primary font-medium mb-2">Choose the winner:</h4>
                        ...
                        {gameState.current_round.submissions && gameState.current_round.submissions.length > 0 ? (
                          ...
                          <div className="text-center text-yellow-300 p-4 bg-yellow-500/20 rounded-lg border border-yellow-500/30">
                            ...
                            No submissions available for judging. This might be a bug.
                            <div className="mt-2">
                              <button
                                onClick={() => {
                                  refreshGameState();
                                }}
                                className="px-3 py-1 bg-red-500 text-text-primary text-xs rounded hover:bg-red-600 transition-colors"
                              >
                                🔄 Force Refresh
                              </button>
                            </div>
                    ...
                    {gameState.current_round.phase === 'card_submission' && 
                     ...
                     gameState.current_round.submissions && gameState.current_round.submissions.length > 0 && (
                      <div className="mb-4">
                        <h4 className="text-text-primary font-medium mb-2">📝 Cards Submitted ({gameState.current_round.submissions.length}):</h4>
                        <div className="space-y-2">
                          {gameState.current_round.submissions.map((submission, index) => (
                            <div
                              key={index}
                              className="w-full bg-blue-500/20 text-text-primary p-3 rounded-lg border border-blue-500/30"
                            >
                              <div className="text-left">
                                <div className="text-sm text-blue-300 mb-1">
                                  {submission.player_username || `Player ${index + 1}`}
                                </div>
                                <div className="text-text-primary font-medium">{submission.card}</div>
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-sm text-accent-green hover:text-accent-green/80 flex items-center"
            ...
            <textarea
              ...
              className="w-full p-4 rounded-lg bg-white/20 text-white placeholder-gray-300 border border-gray-400 focus:border-accent-pink focus:outline-none resize-none"
              rows={3}
            />
            {cardType === 'white' && (
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          {/* Audience Selection */}
          <div>
            <label className="block text-text-primary font-medium mb-2">Audience</label>
            <div className="space-y-2">
          ...
          {/* Topic Selection */}
          <div>
            <label className="block text-text-primary font-medium mb-2">Topic</label>
              ...
              {topicOptions.map((option) => (
                <button
                  ...
                  className={`p-3 rounded-lg font-medium transition-all ${
                    topic === option.value
                      ? 'bg-indigo-600 text-text-primary shadow-lg'
                      : 'bg-text-primary/20 text-background-cream hover:bg-text-primary/30'
                  }`}
                >
                  {option.label}
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-accent-green to-accent-darkGreen text-white font-bold py-4 px-6 rounded-lg hover:from-accent-darkGreen hover:to-accent-green disabled:opacity-50 disabled:cursor-not-allowed transition-all transform hover:scale-105 shadow-lg"
        >
          {loading ? (
      ...
      {context && (
        <div className="flex justify-center my-8">
          <div className="bg-black text-text-primary rounded-2xl border-4 border-white shadow-2xl w-96 min-h-[120px] flex flex-col justify-center items-center p-8 font-bold text-2xl uppercase tracking-wide text-center select-none">
            {context}
          </div>
        </div>
      )}
            ...
            <div className="flex flex-row flex-wrap justify-center gap-6 mt-4">
              {blackCardWhiteCards.map((card) => (
                ...
                >
                  ...
                  <div className="absolute bottom-2 right-2 text-xs bg-gray-200 text-gray-700 px-1.5 py-0.5 rounded-full font-bold shadow-sm">
                    {card.persona}
                  </div>
                  <div className="w-full flex flex-col items-center">
                    <div className="flex items-center space-x-1 mb-2">
                      <span className="px-1.5 py-0.5 bg-green-600/20 text-green-300 rounded text-xs font-medium">
                          ...
                          <button
                            ...
                            className={`w-6 h-6 rounded-full text-xs font-bold transition-all ${
                              gameCardRatings[card.id] === rating
                                ? 'bg-purple-600 text-text-primary shadow-lg scale-110'
            ...
            {generations.map((generation, index) => (
              <div
                ...
                className={`${cardType === 'black' 
                  ? 'bg-black text-text-primary border-4 border-white' 
              ...
              >
                ...
                <div className={`absolute bottom-2 right-2 text-xs px-1.5 py-0.5 rounded-full font-bold shadow-sm ${
                  cardType === 'black' 
                    ? 'bg-white text-black' 
                ...
                <div className="w-full flex flex-col items-center">
                  <div className="flex items-center space-x-1 mb-2">
                    <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${(generation.is_safe ?? generation.generation?.is_safe) ? 'bg-green-600/20 text-green-300' : 'bg-red-600/20 text-red-300'}`}>
                      {(generation.is_safe ?? generation.generation?.is_safe) ? (
                        <><CheckCircle className="inline mr-1" size={12} />Safe</>
                      ) : (
                        <><AlertTriangle className="inline mr-1" size={12} />Filtered</>
                      )}
                    </span>
                    <span className="px-1.5 py-0.5 bg-blue-600/20 text-blue-300 rounded text-xs font-medium">
